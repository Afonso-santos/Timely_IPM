  ...
  <aside :class="{ 'is-expanded': sidebar.isExpanded }">
        ...
        <button
          ...
          @click="
            () => {
              ...
              if (sidebar.isUserExpanded) sidebar.toggleUserMenu()
            }
          "
        >
          <span class="material-icons toggle-icon"> keyboard_double_arrow_right </span>
        </button>
      </div>
    </div>
    ...
    <div class="user-section" ref="userMenuRef" :class="{ collapsed: !sidebar.isExpanded }">
      <div class="user-menu" v-if="sidebar.isUserExpanded">
        <router-link to="/profile" class="menu-option" @click="sidebar.closeUserMenu"
          >Página de Perfil</router-link
        >
      ...
      <div class="user-info">
        ...
        <p class="user-role">
          {{ session.type == 'director' ? 'Diretor(a) de Curso' : 'Aluno(a)' }}
        </p>
      </div>
      <span
